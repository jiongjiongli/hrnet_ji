...
class ji_data(BaseDataset):
    def __init__(self,
        ...
        super(ji_data, self).__init__(ignore_label, base_size,
        ...
        self.label_paths = glob.glob(os.path.join(self.root, "*.png"))
        self.val_split = 0.1
        self.val_num = int(len(self.img_paths) * self.val_split)
        self.train_num = int(len(self.img_paths) - self.val_num)
        ## parse img directory
        self.n_classes = num_classes
        self.imgs = {}
        self.imgnames = []
        for fd in self.img_paths:
            name = fd.split("/")[-1].split(".")[0]
            self.imgnames.append(name)
        np.random.seed(10101)
        np.random.get_state()
        np.random.shuffle(self.imgnames)
        self.train_names = self.imgnames[:self.train_num]
        self.val_names = self.imgnames[self.train_num:]
        self.multi_scale = multi_scale
        self.flip = flip
        self.crop_size = crop_size
        if self.mode == 'train':
            self.files = self.train_names
        if self.mode == 'val':
            self.files = self.val_names
    ...
    def __getitem__(self, index):
        if self.mode == 'train':
            fn = self.train_names[index]
            name = fn
            impth = os.path.join(self.root, fn + ".jpg")
            lbpth = os.path.join(self.root, fn + ".png")
            image = cv2.imread(
                impth,
                cv2.IMREAD_COLOR
            )
            label = np.array(
                Image.open(lbpth).convert('P')
            )
            ...
            # if label.max()>1:
            #     print("max_lable:",label.max())
            return image.copy(), label.copy(), np.array(size), name
        if self.mode == 'val':
            fn = self.val_names[index]
            name = fn
            impth = os.path.join(self.root, fn + ".jpg")
            lbpth = os.path.join(self.root, fn + ".png")
            image = cv2.imread(
...
if __name__ == "__main__":
    ...
    ds = ji_data(r'D:\JWCodes\HRNet-Semantic-Segmentation-HRNet-OCR\865', num_classes=2, mode='train')
    uni = []
    for im, lb, _, name in tqdm(ds):
